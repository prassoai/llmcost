...
func TestCacheWriteTTLSplit(t *testing.T) {
	...
	direct, ok3 := cost(table()["claude-sonnet-4-5"], components{cacheCreation: 1500, cacheCreation1h: 500}, premium{})
...
func TestOpenAINormalization(t *testing.T) {
	...
	direct, ok3 := cost(table()["gpt-5.4"], components{input: 600, cacheRead: 400, output: 100}, premium{})
	if !ok2 || !ok3 || viaAPI != direct {
...
func TestClaudeFastMode(t *testing.T) {
	...
	direct, ok2 := price(table()["claude-opus-4-6"], u)
...
func TestOpenAIRegionalUplift(t *testing.T) {
	...
	direct, ok2 := price(table()["gpt-5.4"], u)
	if !ok || !ok2 || viaAPI != direct {
...
func TestTableInvariants(t *testing.T) {
	for model, r := range table() {
		...
		if r.Fast != nil && r.Fast.Sign() <= 0 {
			t.Errorf("%s: non-positive fast multiplier", model)
		}
		for _, m := range []map[string]*big.Rat{r.Geo, r.RegionalUplift} {
			for key, f := range m {
				if f == nil || f.Sign() <= 0 {
					t.Errorf("%s: non-positive multiplier %q", model, key)
				}
			}
		}
	}
